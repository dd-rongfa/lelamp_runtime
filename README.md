...
| LLM | `openai` 插件接方舟 Ark | 即原版 `livekit-agents[openai]`；新版 chat API，支持 `function_tool`；换 DeepSeek 等只改 `LLM_BASE_URL` |
...
# 可选：LLM_MODEL（默认 doubao-1-5-lite-32k-250115）/ LLM_BASE_URL（默认 Ark；换 DeepSeek 等改这里）/ LAMP_SPEAKER（默认 uranus 女声 Vivi 2.0）
```
